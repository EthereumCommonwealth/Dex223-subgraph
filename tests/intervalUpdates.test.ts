...
describe('dex223 interval data', () => {
  ...
  test('success - create and update dex223DayData', () => {
    // these are the only two fields that get persisted to dex223DayData, set them to non-zero values
    const factory = Factory.load(FACTORY_ADDRESS)!
    const dex223TxCount = BigInt.fromString('10')
    const dex223TotalValueLockedUSD = BigDecimal.fromString('100')
    factory.txCount = dex223TxCount
    factory.totalValueLockedUSD = dex223TotalValueLockedUSD
    factory.save()

    updateDex223DayData(poolEvent)
    ...
    assertObjectMatches('Dex223DayData', dayId.toString(), [
      ...
      ['feesUSD', '0'],
      ['tvlUSD', dex223TotalValueLockedUSD.toString()],
      ['txCount', dex223TxCount.toString()]
    ])
    ...
    assertObjectMatches('Dex223DayData', dayId.toString(), [['txCount', updatedTxCount.toString()]])
  })
})
...
describe('pool interval data', () => {
  ...
  test('success - create and update poolDayData', () => {
    ...
    const dayId = poolEvent.block.timestamp.toI32() / 86400
    const dayStartTimestamp = dayId * 86400
    const dayPoolID = poolEvent.address
      .toHexString()
      .concat('-')
  ...
  test('success - create and update poolHourData', () => {
    ...
    const hourPoolID = poolEvent.address
      .toHexString()
      .concat('-')
...
describe('token interval data', () => {
  ...
  test('success - create and update tokenDayData', () => {
    const token = Token.load(WETH_MAINNET_FIXTURE.addressERC20)!
    ...
    assertObjectMatches('TokenDayData', tokenDayID, [
      ['date', dayStartTimestamp.toString()],
      ['token', WETH_MAINNET_FIXTURE.addressERC20],
      ['volume', '0'],
      ['volumeUSD', '0'],
      ['feesUSD', '0'],
